...
class ComplianceState(BaseModel):
    ...
    ticket_id: Optional[str] = Field(
        default=None, description="Jira Ticket Key (e.g., TRD-101)"
    )
    ticket_owner: Optional[str] = Field(
        default="unknown_user@example.com", description="Email of the Jira Assignee"
    )
    ...
    # --- 2.5 AGGREGATED OUTPUT ---
    # final_decision: Optional[ComplianceResult] = Field(default=None)
    ...
    error_msg: Optional[str] = Field(
        default=None, description="Current validation error to fix"
    )
